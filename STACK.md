...
| 모바일 가상 조이스틱 | **직접 구현** ([touch.js](src/ui/touch.js)) — nipplejs 안 씀 | ✅ |
| 게임패드 | Gamepad API | ⬜ |
...
조사 문서가 "나중에 고치는 비용이 가장 큰 세 가지" 중 1순위로 꼽았던 항목이라 먼저 닫았다.

| 데스크톱 | 모바일 |
|---|---|
| WASD (켜짐/꺼짐) | 왼쪽 절반 드래그 조이스틱 (기울인 만큼 0~1) |
| Shift 달리기 | 조이스틱을 끝까지 밀면 달린다 — 버튼을 더 두면 엄지가 모자란다 |
| 좌클릭으로 대상 지정 | 공격 버튼이 반경 14 안에서 알아서 조준 |
| 우클릭 술법 | 술법 버튼 (반경 20의 적, 없으면 바라보는 앞) |
| Q/E 카메라 회전 | 오른쪽 빈 곳 드래그 |
| 휠 줌 | 두 손가락 핀치 |
| Space/C/E | 회피·방어·상호작용 버튼 |

- 입력은 `Input`이 하나로 모은다. 게임 로직은 어느 쪽에서 왔는지 모른다 (`moveAxis()`)
- 조이스틱은 **엄지가 닿은 자리에 생긴다.** 고정 위치면 손을 옮길 때마다 헛짚는다
- 상호작용 버튼은 쓸 데가 있을 때만 뜬다 — 엄지 자리를 아낀다
- 성능: 개체 수 ×0.55 (종류는 그대로 — 빼면 구역 성격이 달라진다),
  그림자 맵 1024, 픽셀 배율 상한 1.5

**겪은 함정 셋**
1. **미니맵 렌더 위치가 코드에 박혀 있었다** — CSS로 프레임만 옮기면 렌더는 원래 자리에 남는다.
   DOM 프레임의 실제 위치를 읽어 뷰포트로 옮기게 고쳤다 (데스크톱 미세 어긋남도 같이 해결)
2. **미디어 쿼리를 스타일시트 중간에 넣었다** — 같은 특정도면 뒤가 이기므로 덮어써졌다.
   반응형 규칙은 반드시 맨 끝에 둔다
3. **`resize`만으로는 부족하다** — 화면을 돌리거나 주소창이 접힐 때 늦게 오거나 안 오는
   브라우저가 있어 `orientationchange`·`visualViewport.resize`까지 듣는다
...
종류를 늘리는 대신 같은 몹에 성질을 얹는다 (REFERENCE.md §4). 접두사 4종 × 몹 25종 =
조합 100가지인데 코드는 `AFFIXES` 표 하나가 전부다.

| 접두사 | 성질 | 배율 |
|---|---|---|
| 광폭 | 이동 ×1.35 · 공격 간격 ×0.6 | hp ×2.0 · 공격 ×1.3 |
| 재생 | 초당 최대체력 2% 회복 | hp ×2.6 · 공격 ×1.2 |
| 가시 | 근접으로 맞은 피해의 18%를 되돌린다 | hp ×2.4 · 공격 ×1.15 |
| 강골 | 넉백·에어본 면역 | hp ×3.2 · 공격 ×1.25 |

경험치 ×3.2 · 골드 ×3.0 · 혼백 +2. 붙을 확률은 존의 `eliteChance`(초원 7% · 삼림 14%).
보스에는 붙지 않는다.

**추첨은 난수가 아니라 시드다.** `rng(zone.seed + index * 7919)` — 그냥 `Math.random()`을
쓰면 멀티에서 피어마다 다른 개체가 엘리트가 된다. 같은 이유로 **죽을 때 분열하는 접두사는
넣지 않았다**(개체 수가 도중에 바뀌면 스냅샷 인덱스가 어긋난다).

### 귀문 균열 (스포너)

`REFERENCE.md §5`의 "마물 통로"를 그대로 옮겼다. 일괄 리스폰을 대체한다.

- 균열마다 `types` · `interval` · `cap` · `level`을 따로 준다
- 개체는 인덱스 기준으로 균열에 배정되고, 죽으면 **그 균열 둘레에서** 다시 나온다
- **동시 상한**이 곧 성능 예산이다 — 상한을 넘으면 되살아나지 않고 기다린다
- 혼백 `SEAL_COST`개로 봉인하면 그 구멍이 멎고 경험치를 받는다. 봉인은 세이브에 남는다
- 파밍할 구멍만 남기고 지나갈 곳은 막는 선택이 성립한다

**봉인은 방장만 할 수 있다.** 리스폰 계산은 호스트만 하므로 비호스트가 봉인하면
혼백만 날린다.
